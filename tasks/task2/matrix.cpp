...
Matrix::RowMatrix::RowMatrix(double* array, size_t size) : _array(array), _size(size) {}
...
Matrix::Matrix(size_t r, size_t c) : _rows(r), _cols(c), _array(new double[r * c]()) {}

Matrix::Matrix(const Matrix& copy_from) : _rows(copy_from._rows), _cols(copy_from._cols), _array(new double[_rows * _cols])
{
